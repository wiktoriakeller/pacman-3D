...
void Material::SendMaterialToShader(std::shared_ptr<Shader> shader) const {
	...
	for(int i = 0; i < diffuseMaps.size(); i++) {
		...
		if (shader->GetUniformLocation("uMaterial.diffuseMap" + std::to_string(i + 1)) != -1) {
	...
	for(int i = 0; i < specularMaps.size(); i++) {
		...
		if (shader->GetUniformLocation("uMaterial.specularMap" + std::to_string(i + 1)) != -1) {
	...
	for(int i = 0; i < normalMaps.size(); i++) {
		...
		if (shader->GetUniformLocation("uMaterial.normalMap" + std::to_string(i + 1)) != -1) {
			normalMaps[i]->Bind(j);
			j++;
		}
	}
	...
	for(int i = 0; i < heightMaps.size(); i++) {
		...
		if (shader->GetUniformLocation("uMaterial.heightMap" + std::to_string(i + 1)) != -1) {
			heightMaps[i]->Bind(j);
			j++;
		}
	}
