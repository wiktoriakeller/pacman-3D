...
const float diffuseLevels = 4.0;
const float specularLevels = 2.0;
...
vec3 CalculateDirLight(vec3 viewDir, vec3 normal) {
	...
	float level = floor(diffuseStrength * diffuseLevels);
	diffuseStrength = level / diffuseLevels;
	if(uMaterial.useDiffuseColor) {
	...
	level = floor(specularStrength * specularLevels);
	specularStrength = level / specularLevels;
